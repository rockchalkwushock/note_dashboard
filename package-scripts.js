...
module.exports = {
  scripts: {
    ...
    client: {
      description:
        'Build the client code base in production environment with Webpack.',
      default: `${crossEnv(
        'BABEL_ENV=production NODE_ENV=production'
      )} webpack -p --progress --profile --colors --config webpack.config.prod.js`,
      devServer: {
        description:
          'Build & Run the client code base with Webpack-Dev-Server.',
        script: `${crossEnv(
          'BABEL_ENV=development NODE_ENV=development'
        )} webpack-dev-server --colors --profile --config webpack.config.dev.js`
      }
    },
    clean: {

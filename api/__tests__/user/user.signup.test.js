...
describe('POST: /users/sign-up', () => {
  ...
  test('Returns status of 201', async () => {
    ...
    expect.assertions(3)
    try {
      const { body, status } = await mockRoute(
        'signup',
        `${ROOT}/sign-up`,
        null,
  ...
  test('Registering an existing user returns status of 400', async () => {
    ...
    try {
      const {
        body,
        status
      } = await mockRoute('signup', `${ROOT}/sign-up`, null, {
        email: testUser.email,
        password: 'password1'
      })
      expect(status).toEqual(400)
      expect(body.message).toEqual(
        `users validation failed: email: ${testUser.email} already taken!`
      )
  ...
  test('Invalid Email returns status of 400', async () => {
    ...
    try {
      ...
      expect(status).toEqual(400)
      expect(body.message).toEqual(
        'users validation failed: email: user@test is not a valid email!'
      )
